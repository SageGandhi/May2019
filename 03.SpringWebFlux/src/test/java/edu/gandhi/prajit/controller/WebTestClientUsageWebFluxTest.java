...
class WebTestClientUsageWebFluxTest {
	@Autowired
	...
	@BeforeEach
	void beforeEach() {
		this.webTestClient=this.webTestClient.mutate().baseUrl("/api/v1/products").build();
	...
	@Test
	void getAllProducts() {
		when(productRepostory.findAll()).thenReturn(Flux.fromIterable(this.expectedProducts));
